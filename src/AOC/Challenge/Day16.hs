...
solve' :: (Point, Dir) -> Point -> Set Point -> _
solve' start end mp = dijkstra next ((== end) . fst) start
    where
        next (p,d) = M.fromList [((p',d'),c) | ((p',d'), c) <- [((p, d <> East), 1000), ((p, d <> West), 1000), ((p + dirVec d, d), 1)], p' `S.member` mp]
...
day16b = MkSol
    { sParse = toPoints . parseAsciiMap parse
    , sShow  = show
    , sSolve = \(s, e, mp) -> (\(c, ds) -> S.size . S.map fst . M.keysSet $ M.filter (<= c) ds) <$> solve' (s, East) e mp
    }
